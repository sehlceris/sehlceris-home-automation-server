...
export class RemoteControlHttpApiKeyGuard implements CanActivate {
  ...
  canActivate(
    ...
    const bearerToken: string =
      request.query.authorization || HttpGuardHelpers.getBearerTokenFromContext(executionContext);
    ...
    if (key) {
      ...
      this.log.warn(
        `rejected unauthorized remote control command with bearer token: ${bearerToken}`,
      );
      return false;
    }
  }
